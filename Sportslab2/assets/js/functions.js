...
*
* @author StackBros (https://1.envato.market/stackbros-portfolio)
* @version 1.4.2
**/
    ...
    choicesSelect: function () {
       ...
       if (e.isVariableDefined(choice)) {
         ...
         element.forEach(function (item) {
           ...
           var placeHolderVal = item.getAttribute('data-placeholder-val') ? item.getAttribute('data-placeholder-val') : '';
...
quill: function () {
  ...
  // Find all elements that should become a Quill editor
  ...
  editors.forEach(function (el) {
    // Each editor has a data-toolbar attribute like: data-toolbar="#toolbar-uitvoering"
    var toolbarSelector = el.getAttribute('data-toolbar');

    // Safety check: if no toolbar is defined or can't be found, skip this editor
    if (!toolbarSelector || !document.querySelector(toolbarSelector)) {
      console.warn('Quill: toolbar not found for editor', el);
      return;
    }
    //...
    var key = el.id ? el.id : toolbarSelector;

    // Initialize Quill on this element..
   window.quillEditors[key] =  new Quill(el, {
      modules: {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const navLinks = document.querySelectorAll(".nav-link"); // Select all links in the sidebar

  navLinks.forEach(link => {
      // Compare the link's href with the current URL
      if (link.getAttribute("href") === currentUrl || link.getAttribute("href") === currentUrl.split('/').pop()) {
